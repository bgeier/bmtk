...
echo "----------------------------"
$BINDIR/fslint-2.42/fslint/findup ../ \( -path "*/.svn" -o -path "*/ext" -o -path "*/doc" \) -prune -o --summary -name '*.m' 

#Name clashes
echo
echo "Checking for name clashes:"
echo "-------------------------"
$BINDIR/fslint-2.42/fslint/findsn ../ \( -path "*/.svn" -o -path "*/doc" \) -prune -o -C -name '*.m'

echo
echo "Checking for temporary files:"
echo "-------------------------"
$BINDIR/fslint-2.42/fslint/findtf ../ \( -path "*/.svn" -o -path "*/doc" \) -prune -o 
...
$BINDIR/bin/cloc-1.52.pl ../ --exclude-dir=ext --match-f='\.m$' --progress-rate=0 --quiet  --csv|grep 'MATLAB'|awk 'BEGIN{FS=","}{print "Matlab files:",$1,"\nBlank lines:",$3,"\nComments:",$4,"\nLines of code:",$5}'
