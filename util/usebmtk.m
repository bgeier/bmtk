...
%
%   % Full path to BMTK
%   BMTKPATH='/path/to/bmtk';
%   % Uncomment next line to turn on debugging info
%   %VERBOSE=1
%   cd (fullfile(BMTKPATH, 'util'));
%   usebmtk
%
...
if exist(BMTKPATH, 'dir')
    p = genpath(BMTKPATH);
    tp = textscan(p,'%s', 'delimiter', ':');
    tp = tp{1};
    % exclude folders
    ex = tp(cellfun(@length, regexp(tp, 'bmtk/tools|bmtk/doc|bmtk/ext|.svn')) > 0);    
    uselist = setdiff(tp, ex);    
    usepath = strcat(uselist, pathsep);
    usepath = strcat(usepath{:});
    cp = textscan(path, '%s', 'delimiter', ':');
    currpath = cp{1};    
    addpath(usepath)
