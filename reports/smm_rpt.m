...
[input.lss,input.features,~,input.classes] = parse_gct(arg.lss); 
input.likelihood = parse_gct(arg.lss_likeli); % columns are consistent with chem sets file
print_str('Warning :: lss_likeli input is assumed to have same column orientation as chemsets'); 
input.pvalues = parse_gct(arg.lss_pvalues); 
input.ranks = parse_gct(arg.ranks); 
